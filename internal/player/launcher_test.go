...
func TestDetectPlayerCachesResultsAndCanInvalidate(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skip("linux-only")
	}
	dir := t.TempDir()
	...
	if _, found := launcher.detectPlayer(); found {
		t.Fatal("unexpected player in empty PATH")
	}
	fakeBinary(t, dir, "mpv")
	if _, found := launcher.detectPlayer(); found {
		t.Fatal("negative detection result was not briefly cached")
	}
	launcher.invalidateDetectedPlayer()
	if _, found := launcher.detectPlayer(); !found {
		t.Fatal("player was not found after cache invalidation")
	}

	if err := os.Rename(filepath.Join(dir, "mpv"), filepath.Join(dir, "mpv.moved")); err != nil {
		t.Fatal(err)
	}
	if _, found := launcher.detectPlayer(); !found {
		t.Fatal("successful detection result was not cached")
	}
}
...
func TestLaunchDefaultWSLPrefersRundll32(t *testing.T) {
	...
	if _, err := l.launchDefault(url); err != nil {
...
func TestLaunchDefaultWSLExplorerLastResort(t *testing.T) {
	...
	l := NewLauncher("", nil, "", nil)
	cmd, err := l.launchDefault("http://example.invalid/stream")
	if err != nil {
		t.Fatalf("launchDefault failed: %v", err)
	}
...
func TestLaunchDefaultNoOpenerActionableError(t *testing.T) {
	...
	l := NewLauncher("", nil, "", nil)
	_, err := l.launchDefault("http://example.invalid/stream")
	if err == nil {
