...
type LibraryStore struct {
	...
	// gen guards cache promotion against concurrent invalidation.
	gen uint64
}
...
func (s *LibraryStore) set(bucket []byte, key string, value interface{}) error {
	...
	// Durable write first so memory never advertises a write BoltDB rejected.
	if s.db != nil {
...
// setContentPair commits content and its server timestamp atomically.
func (s *LibraryStore) setContentPair(dataKey string, value interface{}, tsKey string, serverTS int64) error {
	...
	if s.db != nil {
		if err := s.db.Update(func(tx *bolt.Tx) error {
			...
			if err := bucket.Put([]byte(dataKey), data); err != nil {
				return err
			}
			return bucket.Put([]byte(tsKey), tsData)
		}); err != nil {
			return err
		}
	}
...
func (s *LibraryStore) getWithTTL(bucket []byte, key string, dest interface{}) bool {
	...
	return decodeWithTTL(wrapper, dest)
}

func decodeWithTTL(wrapper timestamped, dest interface{}) bool {
	...
	if s.db == nil {
		...
		for k, v := range s.cache {
			if strings.HasPrefix(k, string(bucketEpisodes)+":"+prefix) {
				...
				if json.Unmarshal(v, &wrapper) == nil && decodeWithTTL(wrapper, &eps) {
					all = append(all, eps...)
					found = true
				}
			}
...
func (s *LibraryStore) SetWatchState(itemID string, played bool) {
	...
	// []*MediaItem payloads: movie lists, episode lists, playlist items, Cue queue
	...
	s.updateEach(bucketQueue, keyPrefix("items"), patchItemList)
